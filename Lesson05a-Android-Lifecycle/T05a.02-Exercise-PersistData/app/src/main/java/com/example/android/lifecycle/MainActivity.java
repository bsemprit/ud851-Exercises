...
public class MainActivity extends AppCompatActivity {
    ...
    // k (1) Create a key String called LIFECYCLE_CALLBACKS_TEXT_KEY
    String LIFECYCLE_CALLBACKS_TEXT_KEY = "callback";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // k (6) If savedInstanceState is not null and contains LIFECYCLE_CALLBACKS_TEXT_KEY, set that text on our TextView
        if(savedInstanceState != null) {
            if(savedInstanceState.containsKey(LIFECYCLE_CALLBACKS_TEXT_KEY)) {
                String prevText = savedInstanceState.getString(LIFECYCLE_CALLBACKS_TEXT_KEY);
                Log.d(TAG,"Prev lifecycle" + prevText);
                mLifecycleDisplay.setText(prevText);
            }
        }
    ...
    // k (2) Override onSaveInstanceState

    @Override
    protected void onSaveInstanceState(Bundle outState) {
        super.onSaveInstanceState(outState);
        logAndAppend(ON_SAVE_INSTANCE_STATE);
        String currentText = mLifecycleDisplay.getText().toString();
        outState.putString(LIFECYCLE_CALLBACKS_TEXT_KEY, currentText);

    }

    // Do steps 3 - 5 within onSaveInstanceState
    // k (3) Call super.onSaveInstanceState
    // k (4) Call logAndAppend with the ON_SAVE_INSTANCE_STATE String
    // k (5) Put the text from the TextView in the outState bundle
